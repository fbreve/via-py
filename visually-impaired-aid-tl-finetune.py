...
based on visually-impaired-aid-tl-vgg
    ...
    if MODEL_TYPE == 'MobileNet':
        for block in range(1,frozen_blocks+1):    
            for layer in model.layers:
                if (str(block)) in layer.name:
                    layer.trainable = False
        for block in range(frozen_blocks+1,14):           
            for layer in model.layers:
                if (str(block)) in layer.name:
...
with open(log_filename,"a+") as f_log:
    ...
    f_log.write("Model: %s\n" % MODEL_TYPE)
    f_log.write("K-Fold N Repeats: %s\n\n" % KFOLD_N_REPEATS)
...
for frozen_blocks in range(0,MAX_FROZEN+1):
    ...
    for index, [train, test] in enumerate(kf.split(df)):
        ...
        print("\nFrozen Blocks: %s Fold: %i of %i Acc: %.2f%%" % (frozen_blocks, index+1, kfold_n_splits*KFOLD_N_REPEATS, (acc*100)))
