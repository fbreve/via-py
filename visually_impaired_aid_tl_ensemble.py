...
from visually_impaired_aid_tl_ensemble_config import via_config
conf = via_config()
...
def create_model(model_type, instance):
    model_dic = {
        'VGG16': ['vgg16', 224, 224],
        'VGG19': ['vgg19', 224, 224],
    ...
    initializer = tf.keras.initializers.HeUniform(seed=SEED+instance-1)
...
def train_test_model(train_df, test_df, model, preprocessing_function, image_size, split, batch_size, instance):
    ...
    train_df, validate_df = train_test_split(train_df, test_size=0.20, shuffle=True,
                                             stratify=train_df.category, random_state=SEED+instance-1)
    ...
    pred_filename = "./pred_ensemble/conf" + conf.CONF +  "/pred-" + model.name + "-split=" + str(split) + "-inst=" + str(instance) + ".csv"
...
def run_tf(q, model_type, train_df, test_df, index, batch_size, instance):
    ...
    try:    
        model, preprocessing_function, image_size = create_model(model_type, instance)
        acc = train_test_model(train_df,test_df,model,preprocessing_function,image_size, index+1, batch_size, instance)
        q.put(acc)
    except:
        q.put(-1)
        import sys
...
if __name__ == "__main__":
    ...
    # create filenames
    log_filename = "via-tl-ensemble-conf" + str(conf.CONF) + '-' + hostname + ".log"
    csv_filename = "via-tl-ensemble-conf" + str(conf.CONF) + '-' + hostname + ".csv"
    ...
    for instance in range(1,11):        
        ...
        for index, [train, test] in enumerate(kf.split(df)):    
            ...
            while True:
                ...
                p = Process(target=run_tf, args=(q, conf.MODEL_TYPE, train_df, test_df, index, batch_size, instance))
                p.start()
            ...
            print("\nInstance: %i Fold: %i of %i Acc: %.2f%%" % (instance, index+1, kfold_n_splits*kfold_n_repeats, (acc*100)))
            print("Mean: %.2f%% (+/- %.2f%%)" % (np.mean(cvscores)*100, np.std(cvscores)*100))
        
            #record log file
            with open(log_filename,"a+") as f_log:
                f_log.write("Instance: %i Fold: %i of %i Acc: %.2f%% Mean: %.2f%% (+/- %.2f%%)\n" % (
                    instance, index+1, kfold_n_splits*kfold_n_repeats, (acc*100), np.mean(cvscores)*100, np.std(cvscores)*100)) 
